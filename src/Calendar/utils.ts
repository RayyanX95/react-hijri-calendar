...
import type { HijriDate } from "@models";

export const isFirstHijriMonth = (currentHijriDate: HijriDate) => currentHijriDate.month === hijriFirstMonthIndex;

export const isLastHijriMonth = (currentHijriDate: HijriDate) => currentHijriDate.month === hijriLastMonthIndex;

const hijriToGregorian = (hijriDate: HijriDate): Date => {
  try {
    // moment-hijri expects 1-12 for months
    const hijriMoment = moment(`${hijriDate.year}/${hijriDate.month + 1}/${hijriDate.day}`, 'iYYYY/iM/iD');
...
const getDaysInHijriMonth = (hijriYear: number, hijriMonth: number): number => {
  try {
    ...
    const nextMonth = hijriMonth === hijriLastMonthIndex ? hijriFirstMonthIndex : hijriMonth + 1;
    const nextYear = hijriMonth === hijriLastMonthIndex ? hijriYear + 1 : hijriYear;
    const firstOfNextMonth = moment(`${nextYear}/${nextMonth + 1}/1`, 'iYYYY/iM/iD');
...
const getHijriMonthGrid = (currentHijriDate: HijriDate): Date[] => {
  const days: Date[] = [];
  const daysInMonth = getDaysInHijriMonth(currentHijriDate.year, currentHijriDate.month);
...
export const getDaysInMonthArray = (isHijri: boolean, currentActiveViewDate: Date, currentHijriDate: HijriDate): Date[] => {
  if (isHijri) {
